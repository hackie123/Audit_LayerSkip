...
class LayerRouter(nn.Module):
    ...
    def __init__(self, hidden: int, init_bias: float = 1.0, threshold: float = 0.5):
        ...
        self.force_open = False  # set True to bypass routing entirely (vanilla baseline)
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        """
        if self.force_open:
            # Vanilla baseline: layer always runs, matches "standard autoregressive
            # decoding" as measured by ConfLayers/SWIFT (no skipping at all).
            b = h.shape[0]
