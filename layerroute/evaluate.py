...
Supports both backbones via --model_family {qwen,llama}, matching main.py.
...
Usage:
    python evaluate.py --ckpt checkpoints/best_adapters.pt --n_eval 100 --model_family qwen
    python evaluate.py --ckpt checkpoints_llama/best_adapters.pt --n_eval 100 --model_family llama

NOTE (pre-existing, not introduced by the llama port): eval_perplexity's
"restore" step resets EVERY router's bias to cfg.router.init_bias_early,
including layers that should return to init_bias_middle. Since
eval_routing_by_type runs before eval_perplexity, routing metrics are
captured before this happens -- but eval_lora_impact runs AFTER
eval_perplexity, so its forward passes may run with incorrectly-restored
middle-layer biases. Flagged for separate investigation; not fixed here.
"""
...
from utils.config import (SystemConfig, LoRAConfig, RouterConfig, TrainingConfig,
                          DataConfig, QWEN_SPEC, LLAMA_SPEC, derive_middle_bounds)
from models.gated_qwen import GatedQwenLoRA
from models.gated_llama import GatedLlamaLoRA


def _family_spec(model_family: str):
    """Returns (model_cls, spec_dict) for the selected family. Mirrors main.py."""
    if model_family == "llama":
        return GatedLlamaLoRA, LLAMA_SPEC
    return GatedQwenLoRA, QWEN_SPEC
...
def build_model(args):
    model_cls, spec = _family_spec(args.model_family)

    # Middle-layer boundary: Qwen keeps its fixed (8,17); any other family
    # gets a depth-proportional boundary. Mirrors main.py's build_config.
    if args.model_family == "llama":
        middle_start, middle_end = derive_middle_bounds(spec["n_layers"])
    else:
        middle_start, middle_end = 8, 17
...
def eval_routing_by_type(model, samples, tok, cfg, out_dir):
    ...
    print(f"  tool_call: avg_layers={avg(tc['layers_run'])}/{n_layers}  skip={avg(tc['skip_pct'])}%")
    print(f"  planning : avg_layers={avg(pl['layers_run'])}/{n_layers}  skip={avg(pl['skip_pct'])}%")
    return {
...
def eval_perplexity(model, samples, tok, cfg, out_dir):
    ...
    for turns, step_type in samples:
        ...
        with torch.no_grad():
            ...
            # Restore
            # NOTE: see module-level docstring -- this resets ALL routers to
            # init_bias_early, not respecting the middle-band split. Left
            # unchanged here (pre-existing behavior); flagged, not fixed.
            for r in model.routers.routers:
...
def eval_flops(model, routing_metrics, cfg, out_dir):
    print("\n[4/6] FLOPs table...")
    # Family-aware spec (was hardcoded to QWEN_SPEC -- this is the one place
    # a wrong spec would silently corrupt FLOPs numbers without crashing).
    _, spec = _family_spec(cfg.model_family)
...
def eval_lora_impact(model, samples, tok, cfg, out_dir):
    ...
    for turns, step_type in subset:
        ...
        # NOTE: non-finite base loss reflects a real property of the frozen
        # backbone at this skip configuration (verified via debug_nan.py:
        # LoRA is not corrupted -- loss_lora is always finite -- but the
        # frozen path alone can be numerically unstable at layer 0 for some
        # inputs, since the model was never trained/evaluated without LoRA
        # active). Excluding these samples from the average is the honest
        # choice; silently recording nan would poison every downstream mean.
        if not (torch.isfinite(loss_base) and torch.isfinite(loss_lora)):
            n_skipped += 1
            print(f"  [warn] non-finite loss for a {step_type} sample "
                 f"(frozen backbone unstable without LoRA at this skip config) -- excluded")
            continue
    ...
    def avg(lst, k):
        return round(sum(r[k] for r in lst)/len(lst), 4) if lst else float("nan")
    tc = [r for r in rows if r["step_type"]=="tool_call"]
    pl = [r for r in rows if r["step_type"]=="planning"]

    if n_skipped:
        print(f"  ({n_skipped}/{len(subset)} samples excluded due to non-finite base loss)")
    if not tc:
        print("  tool_call: no finite samples -- all excluded, result undefined")
    else:
        print(f"  tool_call: base={avg(tc,'loss_base')}  lora={avg(tc,'loss_lora')}  delta={avg(tc,'loss_delta')}  (n={len(tc)})")
    if not pl:
        print("  planning : no finite samples -- all excluded, result undefined")
    else:
        print(f"  planning : base={avg(pl,'loss_base')}  lora={avg(pl,'loss_lora')}  delta={avg(pl,'loss_delta')}  (n={len(pl)})")

    return {
        "lora_loss_delta_tool": avg(tc,"loss_delta"),
        "lora_loss_delta_plan": avg(pl,"loss_delta"),
        "lora_impact_n_skipped": n_skipped,
    }
# ── 6. Paper summary ───────────────────────────────────────────────

def write_summary(all_metrics, out_dir, args, n_layers, model_name):
    flat = {}
    ...
    flat["model_family"] = args.model_family
    ...
    with open(rpt,"w") as f:
        ...
        f.write(f"  LoRA + Hard-Gated Skip Connections on {model_name}\n")
        f.write("="*60+"\n\n")
        f.write(f"  Checkpoint   : {args.ckpt}\n")
        f.write(f"  Model family : {args.model_family}\n")
        ...
        f.write(f"  Skip candidates    : {gm['n_skip_candidates']}/{n_layers}\n\n")

        f.write("── Adaptive Routing ─────────────────────────────────────\n")
        f.write(f"  tool_call avg layers : {rm['tool_avg_layers_run']}/{n_layers}  (skip {rm['tool_avg_skip_pct']}%)\n")
        f.write(f"  planning  avg layers : {rm['plan_avg_layers_run']}/{n_layers}  (skip {rm['plan_avg_skip_pct']}%)\n")
...
def parse_args():
    ...
    p.add_argument("--model_family",     default="qwen", choices=["qwen", "llama"],
                   help="qwen -> Qwen2.5-0.5B-Instruct. llama -> TinyLlama-1.1B-Chat-v1.0.")
...
if __name__ == "__main__":
    ...
    _, spec = _family_spec(args.model_family)
    model_name = spec["hf_name"]
    tok = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
    if tok.pad_token is None: tok.pad_token = tok.eos_token

    print("\n[Data] Loading eval samples...")
    samples = load_eval_samples(n=args.n_eval)
    ...
    write_summary(all_metrics, out_dir, args, model.n_layers, model_name)
