...
def get_cache_configuration(file_name='skip_layers.json', model_name='llama-2-13b', task_name='cnndm'):
    ...
    with open(file_name) as f:
        ...
        if f'{model_name}_{task_name}' not in data.keys():
            ...
            return data[f'{model_name}_{task_name}']['layers']
...
def dynamic_optimization(model, output_ids, input_past_key_values_data,
            input_current_length_data, statistics, best, position_ids=None):
    # Avoid modifying the input_ids in-place
    generate_ids = output_ids.clone()

    # Compute entropy for each layer and token
    def compute_entropy(attn_outputs, mlp_outputs, statistics):
        attn_confidence_dict = {}
        mlp_confidence_dict = {}
        total_confidence_dict = {}

        for i in range(1, len(model.model.layers)-1):
            # Get attention and MLP hidden states
            h_attn = attn_outputs[i]
            h_mlp  = mlp_outputs[i]

            attn_confidence_dict[i] = compute_module_entropy(h_attn)
            mlp_confidence_dict[i] = compute_module_entropy(h_mlp)

            total_confidence_dict[i] = attn_confidence_dict[i] + mlp_confidence_dict[i]

        skip_layers, local_means, conf, grad, windows = find_skip_layers(total_confidence_dict, statistics)

        plot = False 
        if plot:
            sorted_items = sorted(total_confidence_dict.items(), key=lambda x: x[0])  # by layer index
            layer_ids, kl_values = zip(*sorted_items)
            import matplotlib.pyplot as plt
            plt.rcParams.update({'font.size': 30})
            fig, ax1 = plt.subplots(figsize=(16, 10))
            ax1.plot(layer_ids, conf, marker='o', linestyle='-', color='royalblue', label = "Confidence Values")
            ax1.plot(layer_ids, local_means, color='red', linestyle='--', label = "Local Mean over Window")
            ax1.plot(layer_ids, grad, color='green', linestyle=':', marker='o', label = "Second-Order Gradient")
            ax1.tick_params(axis='y')
            ax1.grid(True, which='both', axis='both', linestyle='--', linewidth=0.6, alpha=0.7)
            ax1.legend()
            ax1.set_xlabel("Layer Index")
            ax1.set_ylabel("Confidence", color='royalblue')
            ax2 = ax1.twinx()
            ax2.plot(layer_ids, windows, color='purple', marker='x', label = "Window size")
            ax2.tick_params(axis='y')
            ax2.set_ylabel("Window Size", color='purple')
            plt.savefig("plots/confidence_llama2-13b_gsm8k.pdf")

        return skip_layers
    
    def compute_module_entropy(logits):
        logits = logits.to(torch.float32)
        logits = torch.clamp(logits, max=88.7, min=np.log(np.finfo(np.float32).tiny))
        probs = F.softmax(logits, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs + 1e-12), dim=-1)
        entropy = torch.mean(entropy)
        max_entropy = torch.log(torch.tensor(probs.size(-1), dtype=torch.float32))
        confidence = (1 - (entropy / max_entropy)).squeeze(0)
        confidence = confidence.to(torch.float16).cpu().detach().numpy()
        return confidence

    def adjust_to_mean_half(x):
        x = np.asarray(x)
        mu = x.mean()
    ...
    def find_skip_layers(confidence_dict, statistics):

        count_consecutive_equal = lambda a: np.argmax(a != a[0]) or len(a)

        sorted_items = sorted(confidence_dict.items()) 
        layer_indices, confidence_values = zip(*sorted_items)
        confidence_values = np.array(confidence_values)
        confidence_values = (confidence_values - np.mean(confidence_values)) / (np.std(confidence_values) + 1e-6)
        grad = np.gradient(np.gradient(confidence_values.flatten(), 1), 1)
        count = count_consecutive_equal(grad)
        min_val = np.min(grad)
        max_val = np.max(grad)
        grad = (grad - min_val) / (max_val - min_val + 1e-8)
        grad = adjust_to_mean_half(grad)
        ...
        lam = statistics["lambda"]
        window_alpha = 5*count
        if "qwen" in statistics["model_id"]:
            window_alpha = min(num_layers//3, 5*count)
        for i in range(len(confidence_values)):
                window_scale = int(window_alpha*(grad[i])*(((i+1)/num_layers)))
                window_size = base_window + window_scale
                windows.append(window_size)
                half_window = window_size // 2
                start = max(0, i - half_window)
                end = min(len(confidence_values), i + half_window + 1)

                local_confidence_window = np.concatenate([confidence_values[start:i], confidence_values[i+1:end]])
                
                local_confidence_mean = local_confidence_window.mean() if len(local_confidence_window) > 0 else 0
                local_confidence_std = local_confidence_window.std() if len(local_confidence_window) > 0 else 0
                window_threshold = local_confidence_mean-lam*local_confidence_std
    ...
    with torch.inference_mode():
        with model.self_validate():
            step_end = statistics["context_window"]
            model.model(input_ids=generate_ids[:, -step_end:],
                                                attention_mask=None,
                                                past_key_values=input_past_key_values,
                                                position_ids=position_ids)
                
            attn_outputs, mlp_outputs=model.model.get_module_intermediate()
            layers_skipped = compute_entropy(attn_outputs, mlp_outputs, statistics)

    model.set_skip_layers(layers_skipped)
    logging.info('Opt_iter {} with {} layers skipped'.format(statistics["opt_iter"], len(layers_skipped)))
    logging.info("Number of layers skipped: {}".format(len(layers_skipped)))
    statistics["opt_iter"] += 1
    if statistics["opt_iter"] >= statistics["max_opt_iter"]:
        model.set_skip_layers(best)
        statistics["optimization"] = None  # stop optimization
        logging.info("\n" + "=" * 30 + 'Optimization Stopped because the optimization iter reaches the maximum!' + "=" * 30 + "\n")
        logging.info("Final Skipped Layer Set: {}".format(model.get_skip_layers()))
        logging.info("Final Skipped Layer Set Size: {}".format(len(model.get_skip_layers())))
    return layers_skipped
