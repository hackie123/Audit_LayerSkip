...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
            ...
            position_ids: Optional[torch.LongTensor] = None,
            past_key_value: Optional[Tuple[torch.Tensor]] = None,
            draft_skip_mask: torch.Tensor = None,
        ...
        recorded_mlp_output, recorded_attn_output = None, None
        if self.training:

            if enabled_draft and draft_skip_mask[self.layer_id].item():
                pass
            else:
            ...
            if enabled_draft and self.layer_id in _skip_layer_id_set:
                ...
                recorded_attn_output = hidden_states
            ...
            if enabled_draft and self.layer_id in _skip_layer_id_set:
                ...
                hidden_states = self.post_attention_layernorm(hidden_states)
                hidden_states = self.mlp(hidden_states)
                recorded_mlp_output = hidden_states
                hidden_states = residual + hidden_states

        outputs = (hidden_states,)
        if enabled_val and not enabled_draft:
            self.set_module_intermediate(recorded_mlp_output, recorded_attn_output)
    ...
    def set_module_intermediate(self, attn_output, mlp_output):
        global _attn_outputs
        global _mlp_outputs
        _attn_outputs[self.layer_id] = attn_output[0] if isinstance(attn_output, tuple) else attn_output
        _mlp_outputs[self.layer_id] = mlp_output[0] if isinstance(mlp_output, tuple) else mlp_output
...
class LlamaModel(_LlamaModel):
    ...
    def reset_module_intermediate(self):
        global _attn_outputs
        global _mlp_outputs
        _attn_outputs, _mlp_outputs = {}, {}

    def get_module_intermediate(self):
        return _attn_outputs, _mlp_outputs
    ...
    def forward(
        ...
        if enabled_val and not enabled_draft:
            self.reset_module_intermediate()
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    draft_skip_mask=draft_skip_mask,
                )
...
class LlamaForCausalLM(_LlamaForCausalLM):
    ...
    @contextmanager
    def self_validate(self, enabled=True, *args, **kwds):
        global enabled_val
        enabled_val = enabled
        try:
            yield None
    ...
    def set_skip_layers(
            self, skip_layer_id_set=None
    ):
        if skip_layer_id_set is not None:
            global _skip_layer_id_set
            _skip_layer_id_set = skip_layer_id_set

    def get_skip_layers(self):
        return _skip_layer_id_set
