...
def clip_input(tokenizer, prompt, task_name, max_new_tokens=512, tree_length=250, max_output_length=4096, prompt_shots=None):
    ...
    if task_name == 'cnndm':
        ...
        inputs = tokenizer(prompt_shots + f"Q: {prompt['question'].strip()}\nA:", return_tensors='pt').to("cuda")
    elif task_name == 'tinystories':
        inputs = tokenizer(
            prompt_shots + "Now, continue the following story:\n" + prompt['text'].strip()[100:],
            return_tensors='pt').to("cuda")
    elif task_name == 'wmt14_de-en':
        inputs = tokenizer(f"Translate German to English:\nGerman: {prompt['translation']['de']}\nEnglish:", return_tensors = 'pt').to("cuda")
    elif task_name == 'alpaca':
        inputs = tokenizer(f"""### Instruction: {prompt['instruction']} ### Input: {prompt['input']} ### Response: """, return_tensors='pt').to("cuda")
    else:
...
def load_data(task_name, seed,  data_num=10):
    ...
    if task_name == 'cnndm':
        ...
        # few_shot_examples = [shots[i] for i in range(n_shot)]
        for i in range(n_shot):
            prompt_shots += f"Q: {shots[i]['question'].strip()}\nA: {shots[i]['answer'].strip()}\n" + "\n"
    elif task_name == 'tinystories':
        n_shot = 1
        data = load_dataset('roneneldan/TinyStories', split='validation').shuffle(seed=seed).select(range(data_num))
        shots = load_dataset('roneneldan/TinyStories', split='train').shuffle(seed=seed).select(range(n_shot))
        for i in range(n_shot):
            shot_story = shots[i]['text'][50:].strip()
            prompt_shots += f"""Here is an example of a story:
                    {shot_story}
                    """
    elif task_name == 'wmt14_de-en':
        data = load_dataset("wmt14", "de-en", split='test').shuffle(seed=seed).select(range(data_num))
    elif task_name == 'alpaca':
        data = load_dataset("tatsu-lab/alpaca", split ='train').shuffle(seed=seed).select(range(data_num))
    else:
        logging.info("This task is not supported.")
...
def get_model_answers(
    ...
    if mean_accepted_tokens > 1:
        best_skip_layer_id_set = model.get_skip_layers()
        best_skip_ratio = len(best_skip_layer_id_set) / (model.config.num_hidden_layers - 2)
        with open(os.path.expanduser(answer_file), "a") as fout:
            ans_json = {
                ...
                "Best Layer Set": best_skip_layer_id_set,
            }
            fout.write(json.dumps(ans_json) + "\n")
